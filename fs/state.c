...
int inode_delete(int inumber) {
    ...
    pthread_rwlock_rdlock(&(inode_table[inumber].inodeLock));

    if ((inumber < 0) || (inumber > INODE_TABLE_SIZE) || (inode_table[inumber].nodeType == T_NONE)) {
        printf("inode_delete: invalid inumber\n");
        pthread_rwlock_unlock(&(inode_table[inumber].inodeLock));
        return FAIL;
    } 

    inode_table[inumber].nodeType = T_NONE;
    pthread_rwlock_wrlock(&(inode_table[inumber].inodeLock));
    inode_table[inumber].status=0;
...
int get_inodeStatus(int inumber){
    return inode_table[inumber].status;
...
void inode_print_tree(FILE *fp, int inumber, char *name) {
    ...
    if (inode_table[inumber].nodeType == T_DIRECTORY) {
        fprintf(fp, "%s", name); //removido o \n para tirar o primeiro enter do ficheiro a seguir ao %s
        for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
            if (inode_table[inumber].data.dirEntries[i].inumber != FREE_INODE) {
